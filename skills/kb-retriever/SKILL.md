...
> ⚠️ **Python 运行环境接线（本机约定）**：本 Skill 及 references 中所有 Python 代码（pdfplumber / pandas / pypdf / pdf2image 等）依赖装在专用 venv `~/.venvs/kb`，系统默认 `python3` **没有**这些包。执行任何 Python 代码前，先 `source ~/.venvs/kb/bin/activate`，或直接用绝对路径 `~/.venvs/kb/bin/python` 代替 `python3`。若该 venv 不存在，先向用户确认再创建。
